...
def calculate\_complex\_meal\_nutrition(meal\_name):
\# ערכים דיפולטיביים
...
translated\_data\_global = {}
original\_full\_json = {}

def translate\_value(value, key=None):
if key == "notes" and not value:
return value
if isinstance(value, str):
if value in translation\_cache:
return translation\_cache\[value]
hebrew\_chars = any('\u0590' <= c <= '\u05FF' for c in value)
if hebrew\_chars:
try:
result = translator.translate(value, "English")
translation\_cache\[value] = result.result
return result.result
except:
return value
return value
elif isinstance(value, dict):
return {k: translate\_value(v, k) for k, v in value.items()}
elif isinstance(value, list):
return \[translate\_value(item) for item in value]
else:
return value

def extract\_food\_nutrition(food\_name):
\# קודם בדוק התאמה מדויקת
if food\_name in nutrition\_db:
return nutrition\_db\[food\_name]
\# השתמש במחשבון ארוחות מורכבות
return calculate\_complex\_meal\_nutrition(food\_name)

def upload\_and\_process(file\_obj):
global translated\_data\_global, original\_full\_json
try:
with open(file\_obj.name, 'r', encoding='utf-8') as f:
content = f.read()
...
def prepare\_medication\_and\_mood\_data(data, mood\_field):
"""
...
def prepare\_symptom\_and\_mood\_data(data, mood\_field):
...
def generate\_medication\_insights(medication\_df, mood\_df):
...
def generate\_symptom\_insights(symptom\_df, mood\_df, mood\_field):
...
def analyze\_activity\_patterns(data, mood\_field):
...
def analyze\_medication\_patterns(data, mood\_field):
"""
...
def analyze\_symptom\_patterns(data, mood\_field):
"""
...
def activity\_analysis\_summary(mood\_field):
if not translated\_data\_global:
...
def medication\_analysis\_summary(mood\_field):
"""
...
def nutrition\_analysis\_summary(mood\_field):
if not translated\_data\_global:
return "Please upload and process data first."
...
def symptom\_analysis\_summary(mood\_field):
"""
...
def upload\_json(file\_obj):
global translated\_data\_global
if file\_obj is None:
return None, "❌ No file uploaded."
try:
\# נשתמש בפונקציה המקורית שכוללת תרגום וניתוח תזונתי
processed\_file, status = upload\_and\_process(file\_obj)
return processed\_file, status
except Exception as e:
...
app.launch(server\_name='0.0.0.0', server\_port=port)
